...
def next_frame_callback(vis_lidar_pc):
    vis_lidar_pc.capture_screen_image("/home/workspace/results/s1ex2/im1.jpeg",do_render=True)
    vis_lidar_pc.close()
...
def show_range_image(frame, lidar_name):
    ...
    ri_range = ri[:,:,0]
    #ri_rnage = ri_range*255/(np.amax(ri_range) - np.amin(ri_range))
...
def bev_from_pcl(lidar_pcl, configs):
    ...
    bev_disc = (configs.lim_x[1]-configs.lim_x[0])/configs.bev_height

    ## step 2 : create a copy of the lidar pcl and transform all metrix x-coordinates into bev-image coordinates    
    lidar_pcl_cpy = np.copy(lidar_pcl)
    lidar_pcl_cpy[:,0] = (np.floor(lidar_pcl_cpy[:,0]/bev_disc))
    lidar_pcl_cpy[:,0] = np.int_(lidar_pcl_cpy[:,0])
    # step 3 : perform the same operation as in step 2 for the y-coordinates but make sure that no negative bev-coordinates occur
    lidar_pcl_cpy[:,1] = np.floor(lidar_pcl_cpy[:,1]/bev_disc) + (configs.bev_width+1)/2
    lidar_pcl_cpy[:,1] = np.int_(lidar_pcl_cpy[:,1])
    ...
    intensity_map[np.int_(lidar_pcl_top[:,0]),np.int_(lidar_pcl_top[:,1])] = lidar_pcl_top[:,3]/(np.percentile(lidar_pcl_top[:,3],98) - np.percentile(lidar_pcl_top[:,3],2))
    #
